...
- ~~**Import from another tracker (CSV/JSON)**~~ **DONE for Grouvee** — the other half of the import story, and the one that carries *progression* rather than ownership: status, score, dates, hours. `/import` uploads an export, shows every game and what we made of it, and writes nothing until confirmed. Grouvee turned out to carry **IGDB's own game id on 606 of 608 rows**, so the fuzzy matcher the spec called "the actual hard part" is not on this path at all and is deferred to the first preset without ids (ADR [0037](docs/decisions/0037-a-tracker-import-carries-history.md)). A tracker supplies a finish date where a platform cannot, so `Played` with one lands in Finished and the rest stay status-less — [0026](docs/decisions/0026-a-library-import-records-ownership-not-history.md)'s own answer for its ambiguous bucket. Playthroughs import with their dates and hours but **no type**, because a typed run feeds the community medians and the source's completion field is a default rather than its owner's answer. Still no `UserGameEvents`. **Next presets** — HowLongToBeat, Backloggery, Completionator, Darkadia — need a flat-CSV `IImportSource` plus the matcher, and no new commit path. Free, per the spec's §8 argument; the pending-job cap is a resource bound, not an entitlement.
