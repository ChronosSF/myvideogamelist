...
- ~~**Reviews**~~ **DONE for writing one.** One review per game, with a spoiler flag, an optional pointer to the playthrough it is about, and a public/private choice that ships now rather than being defaulted later — a default is a consent decision. **Still open:** anywhere to read somebody else's, which needs public profiles (Tier 2 community signal).
...
- **Community signal** — *partly unblocked*: **community completion times ship**, as a median per tier with the number of playthroughs behind it and a display floor, beside IGDB's own figures on the game page (ADR [0025](docs/decisions/0025-playthroughs-and-reviews.md)); and review text with spoiler tags is now *written* and stored. **Still open:** somewhere to read other people's reviews, which needs public profiles first, plus site-wide average score, a score histogram and helpful-votes.
...
| H6 | **Stats strip** — finished this year, hours logged, current streak | Fully unblocked and **not built**: finishes, the streak and now hours logged all come from `/api/user/stats` (ADR [0025](docs/decisions/0025-playthroughs-and-reviews.md)). What is missing is the strip itself, which waits on H1 forking the home page on `user` |
